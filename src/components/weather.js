...
// function Weather() {
//     // API 키 및 URL 설정
//     const WEATHER_API_KEY = "gDPIDuwpKRR1jcaE7qw%2BHclN%2FXEwsjG6GlL%2FgFEQvIRp%2Bg7h4c1B0K2LOvpUyXKOXTO6hOcPPQP1ll93s7wB1g%3D%3D"
...
//     let month = (td.getMonth() + 1) < 10 ? "0" + (td.getMonth() + 1) : td.getMonth() + 1;
...
//                     else y_date = 30;
//                 }
//             } else y_date = (td.getDate() - 1) < 10 ? "0" + (td.getDate() - 1) : (td.getDate() - 1);
//         }
...
//                 date = 31;
//             } else {
//                 month = (td.getMonth()) < 10 ? "0" + td.getMonth() : td.getMonth();
//                 if (month in [2, 4, 6, 9, 11]) date = 31;
//                 else if (month == "03") date = 29;
//                 else date = 30;
//             }
//         } else date = (td.getDate() - 1) < 10 ? "0" + (td.getDate() - 1) : (td.getDate() - 1);
//     }
//
//     // console.log(today);
//     // console.log("shrtTime : " + shrtTime);
//
//     useEffect(() => {
//         requestWeather();
//     }, []);
...
//                 fetch(`${WEATHER_URL}getVilageFcst?serviceKey=${WEATHER_API_KEY}&${WEATHER_PARAMETER}&base_date=${today}&base_time=0200&nx=${lat}&ny=${lng}`);
...
//         try {
//             fetch(`${WEATHER_URL}getUltraSrtFcst?serviceKey=${WEATHER_API_KEY}&${WEATHER_PARAMETER}&base_date=${shrtToday}&base_time=${shrtTime}&nx=${lat}&ny=${lng}`)
//                 .then(response => response.json())
//                 .then(data => shrtWeathers = data.response.body.items.item)
//                 .then(data => {
//                     console.log("shrtWeathers");
//                     console.log(shrtWeathers);
//                     for (let i = 0; i < 60; i++) {
//                         if (shrtWeathers[i].category == "PTY") {
//                             // 강수형태 데이터 처리
//                             console.log("PTY catch : " + i);
//                             if (Number(shrtWeathers[i].fcstTime) < 1200) fcstTime.push("오전 " + shrtWeathers[i].fcstTime.substr(0, 2) + "시");
//                             else fcstTime.push("오후 " + (Number(shrtWeathers[i].fcstTime.substr(0, 2)) - 12) + "시")
//                             if (shrtWeathers[i].fcstValue == "0") pty.push("맑음");
//                             else if (shrtWeathers[i].fcstValue in ["1", "2", "5", "6"]) pty.push("비")
//                             else pty.push("눈")
//                         } else if (shrtWeathers[i].category == "T1H") {
//                             // 기온 데이터 처리
//                             console.log("T1H catch : " + i);
...
//
//     // 중기 예보
//     /* const getMidWeather = async (lat, lng) => {
//         try {
//             const shrtResponse = await
//                 fetch(`${MID_WEATHER_URL}getMidLandFcst?serviceKey=${WEATHER_API_KEY}&${WEATHER_PARAMETER}&regId=11B00000&tmFc=${midTime}`);
//             const shrtJson = await shrtResponse.json();
//             setShrtWeathers(await shrtJson.response.body.items.item);
//         } catch (e) {
//             console.log(e);
//         }
//     }
//
//     // 중기 기온
//     const getMidTemp = async (lat, lng) => {
//         try {
//             const shrtResponse = await
//                 fetch(`${MID_WEATHER_URL}getMidTa?serviceKey=${WEATHER_API_KEY}&${WEATHER_PARAMETER}&regId=11B10101&tmFc=${midTime}`);
//             const shrtJson = await shrtResponse.json();
//             setShrtWeathers(await shrtJson.response.body.items.item);
//         } catch (e) {
//             console.log(e);
...
//         try {
//             const response = await
//                 fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${GOOGLEMAP_KEY}`);
//             const json = await response.json();
...
//                         ))}
//                     </div>
//                     {/* <div id='10days weather forecast'>
//                         <p>10일간의 일기예보</p>
//                     </div> */}
//                 </div>
//             </section>
...
//                 >
//                     <source src={bgChangeByClimate()} type="video/mp4" />   {/* 날씨에 따라 비디오 소스 설정 */}
