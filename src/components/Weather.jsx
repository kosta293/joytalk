...
function Weather() {
    ...
    const getWeather = async (lat, lng) => {
        try {
            const response = await fetch(`${WEATHER_URL}getVilageFcst?serviceKey=${WEATHER_API_KEY}&${WEATHER_PARAMETER}&base_date=${today}&base_time=0200&nx=${lat}&ny=${lng}`);
                    ...
                    for (let i = 0; i < 60; i++) {
                        if (shrtWeathers[i].category === "PTY") {
                            if (Number(shrtWeathers[i].fcstTime) < 1200) {
                                fcstTime.push("오전 " + shrtWeathers[i].fcstTime.substr(0, 2) + "시");
                            } else {
                                fcstTime.push("오후 " + (Number(shrtWeathers[i].fcstTime.substr(0, 2)) - 12) + "시");
                            }
    ...
    const getGeo = async (lat, lng) => {
        try {
            const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${GOOGLEMAP_KEY}`);
            const json = await response.json();
